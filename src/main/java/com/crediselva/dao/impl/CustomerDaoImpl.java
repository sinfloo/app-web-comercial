...
public class CustomerDaoImpl implements CustomerDao {
	...
	public CustomerResponse getObjectForDocument(String document) {
		...
		try {
			...
			if (response.length() > 2) {
				customer = mapper.readValue(response, new TypeReference<List<CustomerResponse>>() {}).get(0);
			}
		} catch (Exception e) {
	...
	public List<CustomerResponse> getAll() {
		...
		try {
			...
			SqlParameterSource parameterSource = new MapSqlParameterSource(map);
			Map<String, Object> mapCustomer = call.execute(parameterSource);
			customers=mapper.readValue(mapper.writeValueAsString(mapCustomer.get("#result-set-1")).toString(), new  TypeReference<List<CustomerResponse>>(){});		
		} catch (Exception e) {
			LOGGER.log(Level.SEVERE, "Error al Obtener Datos",e);
		}
